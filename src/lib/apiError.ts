...
/** Error class names we classify without importing their modules
 *  (apiError stays dependency-free so it can be used from any route). */
const KNOWN_ERROR_CODES: Record<string, string> = {
  DatabaseConfigError: "db:config",
  ImageEngineUnavailableError: "engine:sharp",
};

function knownCode(e: unknown): string | null {
  let cur: unknown = e;
  for (let hops = 0; hops < 4 && cur instanceof Error; hops++) {
...
export function apiError(e: unknown, fallback = "server_error"): NextResponse {
  ...
  let status = 500;
  if (known) {
    // configuration / capability problem, not a server bug: 503 tells the UI
    // (and uptime tools) to retry after the deployment is fixed.
    code = known;
    status = 503;
  } else if (state && /^[02458]/.test(state)) {
  ...
  return NextResponse.json({ error: `${code}: ${msg.slice(0, 300)}`, fallback }, { status });
}
